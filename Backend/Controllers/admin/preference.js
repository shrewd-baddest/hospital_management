...
export const updatePreference = async (req, res) => {
  const { volume, isAutoUpdateOn, isSoundOn, fontSize, isSwitchOn, theme } =
    req.body;
  ...
  try {
    ...
    if (checkUser.rows.includes(user_id)) {
      result = await pool.query(
        `UPDATE preference SET user_id=$1,volume=$2,isSoundOn=$3,fontSize=$4,isSwitchOn=$5,theme=$6,isAutoUpdateOn=$7 RETURNING *`,
        [
          user_id,
          volume,
          isSoundOn,
