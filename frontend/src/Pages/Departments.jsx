...
const Departments = () => {
  ...
  const handleSearch = () => {
    axios
      .post(
        "http://localhost:3000/webpages/departmentSearch",
        { search },
        {
      ...
      <section>
        {Array.isArray(departments) &&
          departments.map((item, idx) => (
            <div
              id={idx}
              className="grid grid-cols-1 transition duration-500 hover:skew-x-1"
            >
              <div>
                <h1 className="text-xl font-bold">{item.name}</h1>
                <h4 className="flex flex-row gap-1">
                  <UserIcon className="inline w-4 h-4" /> Head:{item.head}
                </h4>
                <h4 className="flex flex-row gap-1">
                  <UserGroupIcon className="inline w-4 h-4" /> Staff:
                  {item.staff}
                </h4>
                <h4 className="flex flex-row gap-1">
                  <BuildingStorefrontIcon className="inline w-4 h-4" />
                  Beds:{item.Bed}
                </h4>
              </div>
              <div className="flex flex-col gap-[8%]">
                <input
                  type="button"
                  value="Edit"
                  onclick={navigate(`dashboard/addDepartments/${item.id}`, {
                    state: "edit",
                  })}
                />
                <input
